...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  m.def("forward" , &fourier_layer_forward , "FOURIER_N forward  (CUDA)");
  m.def("backward", &fourier_layer_backward, "FOURIER_N backward (CUDA)");
}
...
__global__ void fourier_layer_cuda_forward_kernel(
  ...
  if (i < N){
	...
	for(int d=0; d<d_head; d++){	
	  ...
	  diff = ( p_head_q_i[d] - p_head_k_i[d] ) * p_paramR[n] ;
  ...
  if (i < N_q){
	...
	for( int qii=qi; qii<qlen; qii+=qleni){
	  ...
	  for( int kii=ki; kii<klen; kii+=kleni){
	    diff = ( p_head_q_i[qii*bsz*n_head*d_head] - p_head_k_i[kii*bsz*n_head*d_head] ) *p_paramR[n];
				
	    if(abs(diff)<1.0e-30f) temp=0;
	    else temp =  1.0f/tanf(diff) - 1.0f/diff  ;
	  
	    temp *= p_Y_i[qii*klen*bsz*n_head + kii*bsz*n_head] * p_grad_Y_i[qii*klen*bsz*n_head + kii*bsz*n_head] * p_paramR[n] ;
	...
	atomicAdd( &p_grad_paramR[n], grad_paramR/(p_paramR[n]*p_paramR[n]) );
  }
