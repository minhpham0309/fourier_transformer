...
class FOURIERFunction(torch.autograd.Function):
    ...
    def forward(ctx, head_q, head_k, paramR):
        head_q = head_q.contiguous()
        head_k = head_k.contiguous()
        output = fourier_cuda.forward(head_q, head_k, paramR)
...
class FourierAttn1(nn.Module):
    def __init__(self, n_head, d_model, d_head):
        ...
        self.paramR = torch.randn( (1), device = cuda_device, dtype=torch.float, requires_grad=True)
...
class FourierAttn0(nn.Module):
    def __init__(self, n_head, d_model, d_head):
        super(FourierAttn0, self).__init__()
    ...
    def forward(self, h, mems):
        ...
        QK_distance0 = (head_q.unsqueeze(3) - head_k.unsqueeze(2)) * self.paramR / math.pi 
...
torch.manual_seed(1)
model2 = FourierAttn0(n_head, d_model, d_head)
...
print('\nmodel1.q_net.weight.grad:', model1.q_net.weight.grad[1,4:7])
...
print('\nmodel1.k_net.weight.grad:', model1.k_net.weight.grad[1,4:7])
...
print('\nmodel1.grad:', model1.paramR )
print('model2.grad:', model2.paramR )
print('\nmodel1.paramR.grad:', model1.paramR.grad[0] )
print('model2.paramR.grad:', model2.paramR.grad[0] )
...
print('\ngrad(C1):', C1.grad[0,2,3:6])
print('grad(C2):', C2.grad[0,2,3:6])

print('\ngrad(mem1): ', mem1.grad[2,1,3:6])
