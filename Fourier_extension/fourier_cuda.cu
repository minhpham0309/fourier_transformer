...
__global__ void fourier_layer_cuda_forward_kernel(
  ...
  if (i < N){
	...
	for(int d=0; d<d_head; d++){	
	  ...
	  diff = ( p_head_q_i[d] - p_head_k_i[d] ) * p_paramR[0] ;
  ...
  if (i < N_q){
	...
	for( int qii=qi; qii<qlen; qii+=qleni){
	  ...
	  for( int kii=ki; kii<klen; kii+=kleni){
	    ...
	    diff = ( p_head_q_i[qii*bsz*n_head*d_head] - p_head_k_i[kii*bsz*n_head*d_head] ) *p_paramR[0];
				
	    if(abs(diff)<1.0e-30f) temp=0;
	    else temp =  1.0f/tanf(diff) - 1.0f/diff  ;
	  
	    temp *= p_Y_i[qii*klen*bsz*n_head + kii*bsz*n_head] * p_grad_Y_i[qii*klen*bsz*n_head + kii*bsz*n_head] * p_paramR[0] ;
	...
	atomicAdd( &p_grad_paramR[0], grad_paramR/(p_paramR[0]*p_paramR[0]) );
  }
